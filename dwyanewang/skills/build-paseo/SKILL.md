...
- **Android 继续分两段构建**：Metro/Hermes 段保持 Gradle `--max-workers=1`，避免 Hermes 与 NDK 内存峰值重叠；bundle 成功并确认非空输出后，ARM64 原生编译与封包使用 `--parallel --max-workers=8 -x :app:createBundleReleaseJsAndAssets`，显式禁止第二段重复 Metro/Hermes。
