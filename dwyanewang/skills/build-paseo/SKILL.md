...
- **Android 继续分两段构建**：Metro/Hermes 段保持 Gradle `--max-workers=1`，避免 Hermes 与 NDK 内存峰值重叠；bundle 成功并确认非空输出后，ARM64 原生编译与封包使用 `--parallel --max-workers=8`，让 `createBundleReleaseJsAndAssets` 留在 Gradle task graph 中并命中 `UP-TO-DATE`。第二段 transcript 必须出现 `> Task :app:createBundleReleaseJsAndAssets UP-TO-DATE`；不要用 `-x :app:createBundleReleaseJsAndAssets`，React Native generated-res/assets provider 在该组合下会让 `:app:mapReleaseSourceSetPaths` 失败。
- **两段 Android Gradle 都通过** `profile-build-resources.sh` **运行**，共享本轮唯一的 `$paseo_build_root/.dev/build-profiles/<时间>-<PID>` 输出目录，label 分别为 `android-metro-hermes`、`android-native-assemble`。脚本用 transient cgroup 统计全部 Java/Node/Hermes/CMake 子进程，每秒写 TSV，并在 summary 中给出实际命令墙钟、平均/采样峰值核心数、20 核整机占比、精确内存/swap 峰值；它以 cgroup 内 wrapper 写出的 command-result 为退出码和聚合数据权威来源，systemd 清理超时只记诊断且最多等待 3 秒。完成汇报必须附目录及两段 summary 核心数据，不能用 `systemd` service runtime（含采样收尾宽限）代替 `command_wall_seconds`。
...
清单增删及对应提交（若有）+ 三端产物（路径 + 体积 + mtime）+ Android 两段资源 summary（目录、`command_wall_seconds`、`average_cpu_cores`、`peak_sampled_cpu_cores`、`host_cpu_percent`、`memory_peak_bytes`、`swap_peak_bytes`）+ serve-dist 打印的下载地址。耗时从用户发起 `$build-paseo` 起算并报告真实墙钟；可以补充分阶段数据，但不得用热缓存阶段或剔除排障后的合计代替总时间。除版本化清单维护和源功能分支的必要兼容修复外，不在打包 worktree 修改或提交产品源码。
