...
export class AgentManager {
  ...
  private async primeTimelineFromProviderEvents(
    ...
    // Dedicated history reads never write the committed timeline, so they must not drop it.
    ...
    for (const event of historyEvents) {
      const row = this.writeHistoryTimelineRow(agent.id, event, writeDurableTimeline);
      if (deferredBroadcast) {
  ...
  private writeHistoryTimelineRow(
    agentId: string,
    event: Extract<AgentStreamEvent, { type: "timeline" }>,
    writeDurableTimeline: boolean,
  ): AgentTimelineRow {
    const timelineOptions = event.timestamp ? { timestamp: event.timestamp } : undefined;
    return writeDurableTimeline
      ? this.recordTimeline(agentId, event.item, timelineOptions)
      : this.timelineStore.append(agentId, event.item, timelineOptions);
  }
