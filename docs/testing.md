...
- Global setup accepts Metro as ready only when `/status` returns `packager-status:running`, then fetches the document's scripts so the cold bundle compilation finishes before Playwright's per-test timeout starts. Document and same-origin bundle fetches share one 120-second warmup budget. Progress lines identify the phase, URL, HTTP status, bytes, and elapsed time; failures also include the Metro process state and recent output, so a bundle compile timeout is not misdiagnosed as a listener startup failure. A generic TCP listener is not sufficient readiness evidence. The browser suite uses direct local daemon connections and does not start a relay.
